...
public class MulticastMessageReceiverService extends IntentService {
  ...
  protected void onHandleIntent(Intent intent) {
    ...
    if (Objects.requireNonNull(action).equals(ACTION_LISTEN_FOR_MULTICAST)) {
      isRunning = true;
      try {
  ...
  private Messenger getHandlerMessenger(Intent intent) {
    return (Messenger) Objects.requireNonNull(intent.getExtras()).get(EXTRA_HANDLER_MESSENGER);
  }
  ...
  private void getLocation(byte[] byteDate, DatagramPacket datagramPacket) {
    double[] othersLocation = byteToDouble(byteDate);
