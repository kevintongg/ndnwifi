...
public class NetworkUtil {
  ...
  public static String getMyWifiP2pIpAddress() {
    try {
      Enumeration<InetAddress> inetAddressEnumeration = Objects
          .requireNonNull(getWifiP2pNetworkInterface())
          .getInetAddresses();
  ...
  private static boolean isWifiP2pInterface(NetworkInterface networkInterface)
    ...
    return networkInterface.isUp() && (
        networkInterface.getDisplayName().equals(NETWORK_INTERFACE_NAME) || networkInterface
            .getDisplayName().contains(ALTERNATE_NETWORK_INTERFACE_NAME));
  }

  private static boolean isIpv4Address(InetAddress inetAddress) {
    return !inetAddress.isLoopbackAddress() && InetAddressUtils
